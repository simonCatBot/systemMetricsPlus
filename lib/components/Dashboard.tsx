...
function ColumnHeader({
  ...
  return (
    ...
    >
      ...
      <h3 className={`text-xs font-semibold ${isActive ? "text-foreground" : "text-muted-foreground"}`}>
        {title}
      </h3>
    </button>
  );
    ...
    <Column isActive={isActive} onClick={onClick}>
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className={`text-xs font-bold ${usage > 80 ? "text-red-500" : "text-foreground"}`}>
              {Math.round(usage)}%
            </span>
          </div>
        </div>
              ...
              {coreLoads.map((load, i) => (
                <div key={i} className="flex flex-col gap-1">
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    <span>C{i}</span>
                    <span className={load > 80 ? "text-red-500 font-normal" : ""}>{load}%</span>
                  </div>
      ...
      {(usageUser > 0 || usageSystem > 0) && (
        <div className="mb-3 text-xs">
          <span className="text-xs text-muted-foreground uppercase tracking-wider">Load Breakdown</span>
      ...
      {coreSpeeds && coreSpeeds.length > 0 && (
        <div className="mb-3">
          <span className="text-xs text-muted-foreground uppercase tracking-wider">Per-Core Speeds (GHz)</span>
          <div className="grid grid-cols-8 gap-1 mt-1">
            {coreSpeeds.map((speed, i) => (
              <div key={i} className="text-center">
                <span className="text-[9px] text-muted-foreground">C{i}</span>
                <p className="text-xs font-mono text-foreground">{speed.toFixed(2)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {cache && (
        <div className="mb-3">
          <span className="text-xs text-muted-foreground uppercase tracking-wider">Cache</span>
      ...
      <div className="mb-3 p-3 bg-surface-1 border border-border rounded-lg">
        <div className="flex items-start justify-between mb-2">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs text-muted-foreground uppercase tracking-wider">Graphics Processor</p>
              <p className="text-xs font-semibold text-foreground truncate">{primaryGpu.marketingName || primaryGpu.name}</p>
            </div>
          </div>
          {trainingStatus && (
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className={`text-xs font-bold ${(primaryGpu.usage ?? 0) > 80 ? "text-red-500" : "text-foreground"}`}>
              {primaryGpu.usage ?? 0}%
            </span>
          </div>
        </div>
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className={`text-xs font-bold ${vramStatus.color}`}>
              {vramStatus.percent}%
            </span>
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-3 text-xs text-muted-foreground mb-3">
        {primaryGpu.temperature !== null && primaryGpu.temperature !== undefined && primaryGpu.temperature > 0 && (
          <div className="flex items-center gap-1">
            ...
            {isThrottling && <span className="text-red-500 text-xs">⚠</span>}
          </div>
        )}
    ...
    <Column isActive={isActive} onClick={onClick}>
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="text-center p-2 bg-surface-2 rounded">
          <p className="text-sm font-bold text-green-400">{used.toFixed(2)}</p>
          <p className="text-xs text-muted-foreground uppercase">Used</p>
        </div>
        <div className="text-center p-2 bg-surface-2 rounded">
          <p className="text-sm font-bold text-foreground">{free.toFixed(2)}</p>
          <p className="text-xs text-muted-foreground uppercase">Free</p>
        </div>
        <div className="text-center p-2 bg-surface-2 rounded">
          <p className="text-sm font-bold text-cyan-400">{swapUsed.toFixed(2)}</p>
          <p className="text-xs text-muted-foreground uppercase">Swap</p>
        </div>
      </div>

      {/* Total */}
      <div className="text-center p-3 bg-surface-2 rounded">
        <p className="text-xl font-semibold text-foreground">{total.toFixed(2)} GB</p>
...
function NetworkColumn({
  ...
  return (
    <Column isActive={isActive} onClick={onClick}>
      ...
      {/* Interface Info */}
      <div className="mb-3">
        <p className="text-xs font-medium text-foreground">{primaryInterface.name}</p>
      </div>

      {/* Speed */}
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="text-center p-2 bg-surface-2 rounded">
          <p className="text-sm font-bold text-green-400">{formatSpeed(primaryInterface.rxSec)}</p>
          <p className="text-xs text-muted-foreground uppercase">Download</p>
        </div>
        <div className="text-center p-2 bg-surface-2 rounded">
          <p className="text-sm font-bold text-blue-400">{formatSpeed(primaryInterface.txSec)}</p>
          <p className="text-xs text-muted-foreground uppercase">Upload</p>
        </div>
      </div>
    ...
    <Column isActive={isActive} onClick={onClick}>
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className={`text-xs font-bold ${totalUsage > 80 ? "text-red-500" : "text-foreground"}`}>
            {totalUsage}%
          </span>
        </div>
        <div className="h-2 bg-surface-2 rounded-full overflow-hidden">
  ...
  return (
    <div className="min-h-screen overflow-hidden" style={{ background: "var(--background)" }}>
      ...
      <main className="px-4 py-4">
        <div className="flex gap-4 pb-2" style={{ height: "calc(100vh - 200px)" }}>
          {isLoading || !metrics ? (
